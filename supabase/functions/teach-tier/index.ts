...
- If anything is off-topic, gently redirect to math.`;
    ...
    const strategySection = strategy
      ? `Teach this strategy for ${tierLabel} (${opName}):
"${strategy}"
...
The child has never done this before. Teach ONLY this given strategy. Use the worked example to ILLUSTRATE it, not to teach a different idea. Do NOT invent your own concept or method.`
      : `Teach the skill: ${tierLabel} (${opName})

The child has never done this before. Use the worked example to illustrate how to approach this problem.`;

    const user = `${strategySection}
...
1. intro: 1–2 sentences that introduce and explain the skill or strategy
2. example_walkthrough: 2–4 short sentences showing how to approach this example, using the EXACT given numbers and answer
